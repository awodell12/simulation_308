...
private double satisfactionThreshold;
  ...
  private Pair checkLikeNeighbors(int i, int j, int type) {
    ...
    if (!isLeftEdge && !isTopEdge) {
      if (myCellGrid[i-1][j-1].getType() == type) similarCount ++;
      if (myCellGrid[i-1][j-1].getType() != EMPTY) neighborCount++;
    }
    if (!isTopEdge) {
      if (myCellGrid[i - 1][j].getType() == type) similarCount++;
      if (myCellGrid[i-1][j].getType() != EMPTY) neighborCount++;
    }
    ...
    if (!isLeftEdge) {
      if (myCellGrid[i][j - 1].getType() == type) similarCount++;
      if (myCellGrid[i][j-1].getType() != EMPTY) neighborCount++;
    }
      if(!isRightEdge) {
        if (myCellGrid[i][j + 1].getType() == type) similarCount++;
        if (myCellGrid[i][j+1].getType() != EMPTY) neighborCount++;
